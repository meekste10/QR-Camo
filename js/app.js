...
state.liveQrScale = 1;
state.moduleStyle = "classic";

const NUDGE_STEP = 8;
const PAN_LIMIT = 360;

const DEFAULT_QR_SIZE = "xxsmall";
const DEFAULT_QR_STYLE = "classic";
...
function currentTrackingProps() {
  return {
    ...
    moduleStyle: qrStyleSelect?.value || DEFAULT_QR_STYLE,
...
function resetAll() {
  ...
  if (qrStyleSelect) qrStyleSelect.value = DEFAULT_QR_STYLE;
  ...
  state.qrSafeMaskCanvas = null;
  state.liveQrScale = 1;
  state.moduleStyle = DEFAULT_QR_STYLE;
...
function init() {
  ...
  if (qrStyleSelect) {
    qrStyleSelect.value = DEFAULT_QR_STYLE;
    state.moduleStyle = DEFAULT_QR_STYLE;
  }
  ...
  if (qrStyleSelect) {
    qrStyleSelect.addEventListener("change", () => {
      state.moduleStyle = qrStyleSelect.value || DEFAULT_QR_STYLE;

      track("qr_style_changed", {
